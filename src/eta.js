...
function isETABreach(data) {
  ...
  const deliveryTime = isoDurationToMilliseconds(data.fulfillments[0]["@ondc/org/TAT"])
  const deliveryETA = new Date(createdAt.getTime()+deliveryTime)

  const deliveryFulfillment = data.fulfillments.find(fulfillment => fulfillment.type === "Delivery");

  if (deliveryFulfillment.state.descriptor.code === "Pending") { // Pre-Ship ETA Breach
    return currentTime >= deliveryETA ? 'y' : 'n';
  } else { // Post-Ship ETA breach
