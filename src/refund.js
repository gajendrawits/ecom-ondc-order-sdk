export const calculateRefundAmount = async (
    ...
    if (typeof transaction.amount === 'undefined' || isNaN(parseFloat(transaction.amount))) {
      throw new Error('Invalid transaction amount: must be a number');
    }
  
    const transactionAmount = parseFloat(parseFloat(transaction.amount).toFixed(2));
    ...
    try {
      ...
      if (!protocolCancelResponse.message || 
          !protocolCancelResponse.message.order || 
          !Array.isArray(protocolCancelResponse.message.order.fulfillments)) {
        throw new Error('Invalid protocolCancelResponse structure: missing required fields');
      }
      ...
      const refundedAmountArray = refundedFl.map(
        (fulfillment) => {
      ...
      for (let pnId of pendingRefundIds) {
        ...
        const fulfillment = protocolCancelResponse.message.order.fulfillments.find(
          (f) => f.id === pnId
        );
        ...
        if (isNaN(refundAmount) || refundAmount < 0) {
          throw new Error(`Invalid calculated refund amount for fulfillment ${pnId}`);
        }
        ...
        if (charge?.quote) {
          ...
          const platformFeesTax = parseFloat(charge.quote.taxes?.platformFeesTax || 0);
